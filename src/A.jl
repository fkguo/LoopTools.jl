...
"""
`A0i(id, m^2)`: one-point tensor coefficient for `id`
...
"""
`A0(m^2)`: the scalar one-point one-loop function
...
"""
`Aget(m^2)`: all one-point tensor coefficients, only defined from real `m^2`
"""
